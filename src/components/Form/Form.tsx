...
const validation = (values: Record<string, any>): Errors => {
  const errors: Errors = {};
...
const Form: React.FC<FormStateProps & FormDispatchProps> = ({
  onSubmit,
  status,
  lang,
}) => {
  ...
  return (
    <>
                ...
                <Field name="message">
                  {({ input, meta }) => (
                    <Input
                      ...
                      id={"message"}
                      type={"textarea"}
                      placeholder={t("captions.message")}
                      required={true}
                    />
                  )}
                </Field>
      ...
      />
      <br />
      {status && status === SendingStatus.SUCCESS && (
        <p>Thank you! I will read your message and answer soon!</p>
      )}
      {status && status === SendingStatus.FAILED && (
        <p>Oooops... Something went wrong, try later </p>
      )}
    </>
  );
};
